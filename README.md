...
- 独立桌宠中心：顶部导航可直接进入桌宠功能页，提供动态大预览、9 款形象选择、启停控制、陪伴状态、休息进度和 5 种互动玩法；新增概念图中的小海鸥、奶油猫和像素机器人。
...
测试覆盖了 Gamma 恢复、单实例保护、休息调度、启动清理、桌宠独立页面、9 款皮肤的状态/配饰渲染、眨眼与视线跟随、互动裁切保护、缩略图/高 DPI 缩放和基础 UI 行为。
